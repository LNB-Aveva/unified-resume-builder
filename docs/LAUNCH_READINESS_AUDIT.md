...
| Expired-score cleanup could fail without failing the workflow. | **Resolved — reconciled in branch** | `cleanup` is now a separate job that exits 1 on non-200 (fail-loud). The `ping` health-check job is independent; a cleanup failure no longer poisons the backend-health result. `CRON_SECRET` is set in both GitHub Actions secrets and Vercel (confirmed by owner 2026-08-09). | Retain as separate jobs; preserve fail-loud exit 1 in cleanup job. |
...
| **10 — Observability** | Backend/frontend Sentry are PII-safe; uptime and cleanup failures alert; approved fixed spend is monitored. | **PASS** | Render Starter and HF usage confirmed. Keepalive split into two independent jobs: `ping` (health) and `cleanup` (retention, fail-loud). `CRON_SECRET` set in GitHub + Vercel (2026-08-09). |

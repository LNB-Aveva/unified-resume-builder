...
| 8.7 Publish correct `ads.txt` at the root and verify crawler access. | `frontend/public/ads.txt` | PARTIAL — file exists and serves 200; publisher ID line is still commented out. Replace the placeholder with Google’s required `google.com, pub-XXXXXXXXXXXXXXXX, DIRECT, f08c47fec0942fa0` record after 8.4 (the current scaffold incorrectly uses `ca-pub-`). |
| 8.8 Validate policy, navigation, content, and ad density before submission. | production site | BLOCKED — 2026-08-03 pre-validation passed public navigation, original content, legal-page reachability, consent-default-denied behavior, and separation of the dormant ad component. Final validation requires 8.4-8.7: certified TCF CMP, publisher/slot IDs, correct live `ads.txt`, AdSense privacy disclosures, and actual placements to assess for ad density. |
...
| 11.6 Verify DNS, SSL, environment values, production CORS, and rollback after the final release candidate. | Vercel/Render/DNS/docs | PARTIAL — 2026-08-03 public technical pass: apex/`www`/API DNS and TLS valid; redirects, HSTS, frontend/API health, disabled production docs, request IDs, auth enforcement, canonical URL, production API/Supabase bundle wiring, and CORS verified; latest CI run `30854065886` is green for Backend, Frontend, and E2E. Owner dashboard environment-scope review and coordinated Vercel/Render rollback rehearsal remain. |

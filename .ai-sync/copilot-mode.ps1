# copilot-mode.ps1 — Launch Copilot with a clear working mode
# Usage:
...
$modes = [ordered]@{
    ...
    "3" = @{
        ...
        Description = "Allow workspace edits; ask before commands Copilot does not consider trusted."
...
if (-not $Mode) {
    Write-Host ""
    Write-Host "Choose a Copilot mode" -ForegroundColor Cyan
    Write-Host ""
...
if ($CopilotArguments) {
    $launchArguments += $CopilotArguments
}
...
if ($selectedMode.Name -eq "plan") {
    Write-Host "After Copilot opens, enter /plan or press Shift+Tab." -ForegroundColor Yellow
}
...
Write-Host ("copilot " + ($displayArguments -join " ")) -ForegroundColor DarkGray
Write-Host ""

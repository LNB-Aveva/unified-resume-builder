...
You work alongside **Claude Code CLI** on this repo.
...
3. Run `git log --oneline -15` — look for `[claude]` prefixes
...
## Hard Rules
- **localhost-first:** make changes → show localhost:3000 → wait for user approval → THEN commit/push
...
- **LINT BEFORE COMMIT (mandatory):** before EVERY `git commit`, run BOTH lint commands below and fix all errors. Never commit with lint failures — CI will reject it and it wastes the GitHub Actions budget. Run `git config core.hooksPath .githooks` at session start to enable the auto-enforcing pre-commit hook.
...
- `CLAUDE.md`             — Claude Code instructions (READ ONLY)
